...
# Receiver Stats
player_targets = data %>%
  filter(week <= 3, pass == 1, !is.na(air_yards)) %>%
...
#Filter Target Share
player_targets = player_targets %>%
  filter(target_share >= 0.20 & targets >= 15) %>%
...
player_targets %>%
  ...
  geom_point(color = player_targets$team_color, cex = player_targets$catches/3, alpha = .6) +
...
player_targets$target_share = player_targets$targets/player_targets$team_passes
player_targets = player_targets[,-8]
...
player_targets %>% 
  ...
  gt() %>%
  tab_header(title = 'Receiver Stats') %>% 
  ...
  cols_label(
    ...
    receiving_yards = 'Yds',
    sum_ay = 'Air Yards',
    mean_ay = 'Avg Air Yards'
  ) %>% 
  fmt_number(columns = vars(mean_ay), decimals = 1) %>% 
  ...
  data_color(
    columns = vars(targets, target_share),
    colors = scales::col_numeric(palette = c('green', 'darkgreen'), domain = c(180,380)),
