...
# Week Variable
curr_week = 7
week_interval = curr_week - 3

# Rusher Stats
player_carries = data %>%
  filter(week <= curr_week, week >= week_interval, rush == 1) %>%
  group_by(id, rusher) %>%
  summarize(
    carries = sum(rush),
...
# RB Targets
rb_targets = data %>%
  filter(week <= curr_week, week >= week_interval, pass == 1, !is.na(air_yards)) %>%
...
# Filter
player_carries = player_carries %>%
  filter(carries >= 25 & carries/games >= 10)
         # & carries/games < 24 & targets/games < 8)

# RB Carries vs Targets
player_carries %>%
  ggplot(aes(x = carries/games, y = targets/games)) +
